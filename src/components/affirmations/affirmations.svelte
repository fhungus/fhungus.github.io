<script lang="ts">
    ...
    class AffirmationEntity {
        ...
        constructor(w: number, h: number) {
            let num: 1 | 2 | 3 | 4 = 1;
            let guhhihatejs = range(Math.random(), 0, 1, 0, 3.1);
            if (guhhihatejs > 3) {
                num = 4;
            } else if (guhhihatejs > 2) {
                num = 3;
        ...
        stepFrame(
            ...
            if ((this.y > h && this.vy > 0) || (this.y < 15 && this.vy < 0)) {
                // EPIC magic number....
            ...
            if (mouse.active) {
                ...
                let decent_gaussian = (x: number) => {
                    ...
                    gaussian -=
                        PEAK * 1.2 * Math.exp(-((x ^ 2) / ((RANGE / 10) ^ 2))); // THUNK!
                    return gaussian;
                };
            ...
            if (this.vx > SPEED_LIMIT || this.vx < -SPEED_LIMIT) {
                // theres no math.clamp function, how sad...
                this.vx = SPEED_LIMIT * vx_sign;
            }
            if (this.vy > SPEED_LIMIT || this.vy < -SPEED_LIMIT) {
                // theres no math.clamp function, how sad...
    ...
    onMount(() => {
        ...
        let onTimeout = (tick: number) => {
            ...
            if (active) {
                ...
                const ROUNDING = 1;
                affirmations.affirmations.map((aff, _) => {
                    // highkey should be using a map for these
                    ctx.fillStyle = `rgba(${aff.variant == 1 ? "116,225,195" : aff.variant == 2 ? "198,209,142" : aff.variant == 3 ? "255,255,92" : "245, 202, 199"}, ${aff.opacity})`;
                    let forwards =
                        aff.variant == 1
                            ? "< ><"
                            : aff.variant == 2
                        ...
                        aff.variant == 1
                            ...
                            : aff.variant == 2
                              ...
                              : aff.variant == 3
                                ...
                                : "Ɛ>< >";
                    ctx.fillText(
                        `${aff.vx < 0 ? forwards : backwards}`,
                        Math.round(aff.x / ROUNDING) * ROUNDING,
                        Math.round((aff.y - TEXT_HEIGHT / 2) / ROUNDING) *
                            ROUNDING,
                    );
                });
            }
...
<div
    class="affirmations"
    style="margin-left:{offset.current}px; margin-right:{-offset.current}px;"
>
    <button
        class="affirmations_button"
        onclick={() => {
            active = !active;
    ...
    {#if !active}
        <div transition:blur class="pause_text">
            <!--appears when the fish are paused...-->
            Your fish have been paused.<br /><br />
            This thing is here because I make them do a lot of wacky calculations
            and they might be particulary lag inducing.
        </div>
    {/if}
    <canvas
        bind:this={canvas}
        class={`affirmations_canvas ${!active && "halfopacity"}`}
    ></canvas>
</div>
